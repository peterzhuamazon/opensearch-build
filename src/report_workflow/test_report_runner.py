    ...
    def component_entry(self, component_name: str, ci_group: int = None) -> Any:
        ...
        for config in config_names:
            ...
            try:
                ...
                test_result_osd_xml = [f for f in component_yml["test_result_files"] if f.endswith(".xml")] if self.name == "opensearch-dashboards" else []
            ...
            config_dict["failed_test"] = get_failed_tests(self.base_path, str(self.test_run_id), self.test_type, test_report_component_name, config, self.name, test_result, test_result_osd_xml)
...
def get_failed_tests(base_path: str, test_number: str, test_type: str, component_name: str, config: str,
                     product_name: str, test_result: str, test_result_osd_xml: list) -> typing.List[list]:
    ...
    if product_name == 'opensearch':
        if base_path.startswith("https://"):
            result_path_list.append("/".join([base_path.strip("/"), "test-results", test_number, test_type, component_name, config, "opensearch-integ-test", "index.html"]))
        else:
            result_path_list.append(os.path.join(base_path, "test-results", test_number, test_type, component_name, config, "opensearch-integ-test", "index.html"))
    else:
        if test_result_osd_xml:
            result_path_list = test_result_osd_xml
        else:
            failed_test_list.append("Test Result XML List Not Available")
            return failed_test_list
